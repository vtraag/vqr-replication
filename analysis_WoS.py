...
#%%
conf_interval = 0.95
low_bound = pd.concat(MAD_bootstrap).groupby('GEV').aggregate([lambda x: np.percentile(x, 100*(1 - conf_interval)/2)])
low_bound.columns.set_levels(['lb'], level=1, inplace=True)
up_bound = pd.concat(MAD_bootstrap).groupby('GEV').aggregate([lambda x: np.percentile(x, 100*((1 - conf_interval)/2 + conf_interval))])
...
low_bound = pd.concat(MAPD_bootstrap).groupby('GEV').aggregate([lambda x: np.percentile(x, 100*(1 - conf_interval)/2)])
low_bound.columns.set_levels(['lb'], level=1, inplace=True)
up_bound = pd.concat(MAPD_bootstrap).groupby('GEV').aggregate([lambda x: np.percentile(x, 100*((1 - conf_interval)/2 + conf_interval))])
up_bound.columns.set_levels(['ub'], level=1, inplace=True)
...
MAD.to_csv('../results/MAD_min={0}.csv'.format(min_n_per_institution))
MAPD.to_csv('../results/MAPD_min={0}.csv'.format(min_n_per_institution))
...
plt.grid(axis='y', ls=':')
sns.despine()
plt.savefig('../results/MAD_min={0}.pdf'.format(min_n_per_institution), bbox_inches='tight')
plt.savefig('../results/MAD_min={0}.png'.format(min_n_per_institution), dpi=300, bbox_inches='tight')
...
plt.grid(axis='y', ls=':')
sns.despine()
plt.savefig('../results/MAPD_min={0}.pdf'.format(min_n_per_institution), bbox_inches='tight')
plt.savefig('../results/MAPD_min={0}.png'.format(min_n_per_institution), dpi=300, bbox_inches='tight')
